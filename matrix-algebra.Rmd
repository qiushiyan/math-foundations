...
Let $A$ be an invertible $n \times n$ matrix. For any $\boldsymbol{b}$ $\mathbb{R^n}$, the unique solution $\boldsymbol{x}$ of $A\boldsymbol{x} = \boldsymbol{b}$ has entries given by: 
...
There is a one to one relationship between a linear transformation and a matrix. Let $T: \mathbb{R^n} \rightarrow R^m$ be a linear transformation. Then there exists a **unique** matrix $A$ such that:  
...
A mapping $T: \mathbb{R^n} \rightarrow R^m$ is said to be **onto** R^m if each $\boldsymbol{b}$ in $R^m$ is the image of at least one $\boldsymbol{x}$ in \mathbb{R^n}
```
...
A mapping T: $\mathbb{R^n} \rightarrow R^m$ is said to be **one-to-one** if each $\boldsymbol{b}$ in $R^m$ is the image of *at most* one $\boldsymbol{x}$ in $\mathbb{R^n}$
```
